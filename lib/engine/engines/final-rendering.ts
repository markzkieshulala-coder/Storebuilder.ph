...
export class FinalRenderingEngine extends BaseEngine<unknown, FinalRenderingArtifact> {
  ...
  async execute(input: EngineInput<unknown>): Promise<EngineOutput<FinalRenderingArtifact>> {
    const frontend = input.context.getArtifact<{ pages: Array<{ id: string; route: string; template: string; imports: string[]; meta?: { title?: string; description?: string } }>; entryPoint: string; globalStyles: string; buildConfig: { bundler: string; outputDir: string } }>('frontend');
